    ...
    def _prepare_invoice_line(self, account_lines, invoice):
        ...
        for account_line in account_lines:
            ...
            if account_line.task_id:
                ...
                account_analytic_id = account_line.task_id.project_id.analytic_account_id
            else:
                # amount = account_line.project_id.price_rate
                account_analytic_id = account_line.project_id.analytic_account_id
            # analytic_account_id = self.env['account.analytic.line'].browse(account_analytic_id)
            ...
            if not account:
                raise UserError(
                    _('Please configure Default Income account for Product income: `property_account_income_categ_id`.'))
            line_product = self.env['product.product'].browse(product_id_helpdesk) if product_id_helpdesk else False
            res = {
                'name': _(name),
                'account_id': account.id,
                'price_unit': price_unit,
                # we allready manipulate the total task cost (account_obj.unit_amount*account_obj.amount)
                ...
                'tax_ids': [(6, 0, line_product.taxes_id.filtered(
                    lambda tax: tax.company_id == account_line.company_id).ids)] if line_product else [],
    ...
    def create_timesheet_invoice(self):
        ...
        for res_line in analytic_account_line:
            if res_line.invoiced_created:
                raise Warning(
                    _('Invoice can not be created since some of Timesheet lines are already invoiced so please check.'))
